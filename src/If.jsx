...
export class IfOuter extends Component {
  ...
  constructor(props) {
    ...
    this.filterKids = type =>
      Children.toArray(this.props.children.props.children).filter(
        el => el.type === type
      )
  }
  ...
  componentDidUpdate(lastProps) {
    if (
      lastProps.selector !== this.props.selector ||
...
export class Select extends Component {
    ...
    if (!loaded || !selector(state, selectorProps)) {
      if (!loaded) {
        const promise = new Promise(resolve => {
          const newState = stateContext.unstable_read()
          if (loadedSelector(newState, selectorProps)) {
            resolve()
          }
        })
        promise.redux = true
      ...
      const promise = new Promise(resolve => {
        const newState = stateContext.unstable_read()
        if (selector(newState, selectorProps)) {
          resolve()
        }
      })
      promise.redux = true
  ...
  render() {
    return <stateContext.Consumer>{this.renderChildren}</stateContext.Consumer>
  }
}
    ...
    <SubContext.Consumer>
      {({ selector, loadedSelector, selectorProps }) => {
        return (
          ...
          >
            <Select
              selector={(state, props) => !selector(state, props)}
              loadedSelector={loadedSelector}
              name="Else"
              selectorProps={selectorProps}
            >
